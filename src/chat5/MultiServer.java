...
public class MultiServer {
	...
	public void init() {
		...
		try {
			...
			/* 1명의 클라이언트가 접속할 때마다 접속을 허용(Accept)해주고 동시에 MultiServerT 내부클래스를 
			통해 쓰레드를 생성한다. 해당 쓰레드는 1명의 클라이언트가 전송하는 메세지를 읽어서 Echo 해주는 역할을 
			담당한다. */
			while(true) {
				//클라이언트의 접속 허가
	...
	/* chat4 까지는 init()이 static 이었으나 chat5 부터는 일반적인 멤버메서드로 변경된다. 따라서 인스턴스를
	생성 후 호출하는 방식으로 변경되었다. */
	...
	class MultiServerT extends Thread {
		...
		public void run() {
			...
			try {
				...
				/*
				두번째 부터는 실제 메세지이므로 지속적을 읽어서 Echo 한다.
				 */
				while(in != null) {
					s = in.readLine();
					if(s == null) {
				...
				/* try문의 while문을 탈출하는 것이 해당 쓰레드가 종료된다는 의미이므로 아래와 같이
				쓰레드의 이름을 콘솔에 출력한다. */
